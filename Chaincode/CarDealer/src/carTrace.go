...
func (t *AutoTraceChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	} else if function == "getVehiclePartsByAssembler" { // get Vehicle parts by assembler
		return t.getVehiclePartsByAssembler(stub, args)
	}
...
func (t *AutoTraceChaincode) initVehiclePart(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	var err error
	...
	indexName := "vehiclePart~assembler"
...
// ============================================================
func (t *AutoTraceChaincode) initVehicle(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	var err error
...
// ===============================================
func (t *AutoTraceChaincode) readVehiclePart(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
// ===============================================
func (t *AutoTraceChaincode) readVehicle(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
// ==================================================
func (t *AutoTraceChaincode) deleteVehiclePart(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	var jsonResp string
...
// ==================================================
func (t *AutoTraceChaincode) deleteVehicle(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	var jsonResp string
...
// ===========================================================
func (t *AutoTraceChaincode) transferVehiclePart(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	//   0       1       3
	// "name", "from", "to"
	if len(args) < 3 {
...
// transfer a vehicle part by setting a new owner name on the vehiclePart
// ===========================================================
func (t *AutoTraceChaincode) transferPartToVehicle(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
// ===========================================================
func (t *AutoTraceChaincode) transferVehicle(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	//   0       1       3
	// "name", "from", "to"
	if len(args) < 3 {
		return shim.Error("Incorrect number of arguments. Expecting 3")
	...
	// if sucessful, returns us a byte array we can then us JSON.parse to unmarshal
	message, err := t.transferVehicleHelper(stub, chassisNumber, currentOnwer, newOwner)
	if err != nil {
		return shim.Error(message + err.Error())
	} else if message != "" {
		return shim.Error(message)
...
// ===========================================================
// transferVehicleHelper : helper method for transferVehicle
// ===========================================================
func (t *AutoTraceChaincode) transferVehicleHelper(stub shim.ChaincodeStubInterface, chassisNumber string, currentOwner string, newOwner string) (string, error) {
	// attempt to get the current vehicle object by serial number.
	// if sucessful, returns us a byte array we can then us JSON.parse to unmarshal
...
// ===========================================================================================
func (t *AutoTraceChaincode) getVehiclePartByRange(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
// ===========================================================================================
func (t *AutoTraceChaincode) getHistoryForRecord(stub shim.ChaincodeStubInterface, args []string) peer.Response {
...
func (t *AutoTraceChaincode) setPartRecallState(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	// ==== Trigger event if recall true
	if recall {
		stub.SetEvent("recallEvent", vehiclePartJSONasBytes) 	
	}
...
func (t *AutoTraceChaincode) getVehiclePartsByAssembler (stub shim.ChaincodeStubInterface, args []string) peer.Response {
	var assembler string
	var err error

	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments. Expecting assembler of the vehicle part to query")
	}

	assembler = args[0]
	indexName := "vehiclePart~assembler"
	partIterator, err := stub.GetStateByPartialCompositeKey(indexName, []string{assembler})
	if err != nil {
		return shim.Error(err.Error())
	}
	...
	buffer.WriteString("[")
	first := true
	for partIterator.HasNext() { //Returns true if we are not at the end of the result set resultObj, 
		resultObj, err := partIterator.Next() //Read the next element from the resultSet 
		if err != nil {
			return shim.Error(err.Error())
		} 
		_, components, err := stub.SplitCompositeKey(resultObj.Key) //Does the opposite of CreateCompositeKey, which was used in create index, splitting a composite key into the name and attributes that made it up. The resulting components object is an array of strings, in this case of the form [assembler, serialNumber]
		if err != nil {
			return shim.Error(err.Error())
		}
		//Components should be assembler, serialNum
		if (len(components) < 2) {
			return shim.Error("Index is malformed for car components!")
		}
		// Retrieve the object associated with our key 
		partAsBytes, err := stub.GetState(components[1]) 
		if err != nil { 
			return shim.Error(err.Error()) 
		} 
		if partAsBytes == nil { 
